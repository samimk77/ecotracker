...
function kMeans(points, clusterCount, maxIterations = 20) {
  if (points.length <= clusterCount) return points.map((_, index) => index);
  ...
  for (let iteration = 0; iteration < maxIterations; iteration += 1) {
    ...
    points.forEach((point, pointIndex) => {
      ...
      sums[cluster].x += point[0]; sums[cluster].y += point[1]; sums[cluster].n += 1;
    });

    sums.forEach((sum, cluster) => {
      if (sum.n > 0) centers[cluster] = [sum.x / sum.n, sum.y / sum.n];
    });
    if (!changed) break;
  }
  return assignments;
...
export function buildMergedWards(geojson, targetZoneCount = DEFAULT_ZONE_COUNT) {
  if (!geojson?.features?.length) return { zones: null, markers: [] };

  const zoneCount = Math.max(20, Math.min(25, Math.min(targetZoneCount, geojson.features.length)));
  ...
  const zones = {
    ...
    features: geojson.features.map((feature, index) => {
      ...
      if (!zoneStats.has(zoneId)) {
        zoneStats.set(zoneId, { zoneId, wardCount: 0, x: 0, y: 0 });
      }
